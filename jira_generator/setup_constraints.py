...
class ConstraintBuilder:
    ...
    def _find_target_issue(self, project_key: str, issue_type: str, summary: str) -> Optional[Dict]:
    ...
    def create_constraint(self, constraint_data: Dict) -> Optional[Dict]:
        ...
        existing = self.client.find_issue_by_summary(
            project_key,
            summary,
            ISSUE_TYPE_NAMES['constraint']
        )
        if existing:
            logger.info(f"Constraint already exists: {existing.get('key')} - {summary[:40]}...")
            self.stats['exists'] += 1
            return existing
        ...
        try:
            issue = self.client.create_issue(
                ...
                labels=['constraint', f"guild-{constraint_data.get('guild', 'unknown').lower().replace(' ', '-')}"],
            )
    ...
    def _transition_to_status(self, issue_key: str, target_status: str):
        ...
        for transition_name in transitions:
            try:
                ...
                logger.warning(f"Failed to transition {issue_key} via '{transition_name}': {e}")
                break

    def link_constraint_to_target(self, constraint_key: str, constraint_data: Dict) -> bool:
        """
        ...
        if not target_issue:
            logger.warning(f"Target not found: {target_type} '{target_summary}' in {project_key}")
