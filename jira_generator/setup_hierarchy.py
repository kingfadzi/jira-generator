...
class HierarchyBuilder:
    ...
    def create_portfolio_epic(
        self, project_key: str, summary: str, parent_key: str, description: str = None
    ) -> Optional[Dict]:
    ...
    def build_hierarchy(self) -> Dict:
        ...
        for strat_obj_data in HIERARCHY:
            ...
            for pe_data in strat_obj_data["portfolio_epics"]:
                ...
                if not pe_issue:
                    logger.error(
                        f"  Failed to create PE: {pe_data['summary']}, skipping children"
                    )
                    continue
                ...
                for bo_data in pe_data["business_outcomes"]:
                    ...
                    if not bo_issue:
                        logger.error(
                            f"    Failed to create BO: {bo_data['summary']}, skipping children"
                        )
                        continue
                    ...
                    for feature_data in bo_data["features"]:
                        logger.info(
                            f"      Creating Feature: {feature_data['summary']}"
                        )
