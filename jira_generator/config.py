...
def get_required_env(var_name: str) -> str:
    ...
    if value is None:
        print(f"\n{'=' * 80}", file=sys.stderr)
        print(
            f"ERROR: Missing required environment variable: {var_name}", file=sys.stderr
        )
        print(f"{'=' * 80}", file=sys.stderr)
        print(
            "\nJira configuration is required. Please set environment variables:",
            file=sys.stderr,
        )
...
# Custom Fields for Story Risk Triggers
STORY_TRIGGER_FIELDS = {
    "trigger_external_boundary": {
        "name": "Risk: External Boundary",
        "description": "Does this cross system boundaries, external APIs, or third-party integrations?",
        "type": "com.atlassian.jira.plugin.system.customfieldtypes:select",
        "options": ["Yes", "No"],
    },
    "trigger_sensitive_data": {
        "name": "Risk: Sensitive Data",
        "description": "Does this expose, store, or transfer sensitive data (including residency)?",
        "type": "com.atlassian.jira.plugin.system.customfieldtypes:select",
        "options": ["Yes", "No"],
    },
    "trigger_security_controls": {
        "name": "Risk: Security Controls",
        "description": "Does this change AuthN/authZ, encryption, or security controls?",
        "type": "com.atlassian.jira.plugin.system.customfieldtypes:select",
        "options": ["Yes", "No"],
    },
    "trigger_ml_model_change": {
        "name": "Risk: ML/AI Model Change",
        "description": "Does this add, modify, or retrain ML/AI models?",
        "type": "com.atlassian.jira.plugin.system.customfieldtypes:select",
        "options": ["Yes", "No"],
    },
    "trigger_critical_logic": {
        "name": "Risk: Critical Logic",
        "description": "Does this change calculations, compliance rules, or transaction processing?",
        "type": "com.atlassian.jira.plugin.system.customfieldtypes:select",
        "options": ["Yes", "No"],
    },
}
